module PriceCalculatorHelper
  def self.price_of_item(item, quantity)
    item_price = Item.select(:id, :unit_price, :sales_price, :min_sales_unit).where(name: item).first
    ...
    unless sale_price && min_sales_unit <= quantity
      return { price: cost_without_sales, savings: 0 }
    end
    ...
    return { price: total_cost, savings: cost_without_sales - total_cost }
  end
end
